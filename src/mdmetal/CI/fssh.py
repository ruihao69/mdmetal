...
def compute_hopping_prob(
    ...
    for ll in range(nstates):
        alk_conj = np.conj(c[ll]) * c[kk] 
        prob = - 2.0 * np.real(alk_conj * v_dot_d[ll, kk]) / akk * dt
        hopping_prob[ll] = prob if prob > 0 else 0.0
    return hopping_prob 
...
def verlet_ci_fssh(
    ...
    c, hopping_flag, target_state = quantum_rk4(c, active_state, last_evals, last_v_dot_d, dt*0.5, last_order)
